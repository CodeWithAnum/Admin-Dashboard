...
interface Order {
  ...
  orderDate: string;
  status: string | null;
  cartItems: { name: string; image: string }[] | null; // Ensure cartItems can be null
}
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    client
      ...
      .then((data) => {
        console.log("Fetched Orders:", data); // Debugging log to check fetched data
        setOrders(data);
      })
      .catch((error) => {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    if (!orderId) {
      console.error("Invalid order ID for status update");
      return;
    }
